...
def test_add_junction_and_outfall_update_counts():
    ...
    assert model.add.node.junction("J1", invert_elevation=10.0, max_depth=3.0) == "J1"
    assert model.add.node.outfall("OUT1", invert_elevation=9.0, type="FREE") == "OUT1"
...
def test_add_rain_gage_and_subcatchment_with_default_coordinates():
    model = _model_with_minimal_network()
    ...
    # The generated default display polygon is centered on the minimum mapped point.
...
def test_generic_fallback_and_save_after_add_remove(tmp_path):
    model = swmm(new="SI")
    model.add_element("node", "junction", "J1", invert_elevation=10.0, max_depth=3.0)
    model.add_element("node", "outfall", "OUT1", invert_elevation=9.0, type="FREE")
